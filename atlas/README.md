...
v0.4는 **인구 규모 / 연간 소비액** Lens에 외부 조사 기반 인구 보정과 2025 온라인 소비 기준을 연결합니다. 20개 산업 중 14개 산업에서 일부 지출 범위를 비교할 수 있습니다. 전국 온라인 거래액을 합성 성인 프로필에 배분한 값이며, 실제 구매자 평균·전체 가계 소비·회사 매출이 아닙니다. [현재 산식·출처·제한](docs/EXTERNAL_SPEND_AND_POPULATION.md)을 참고하세요.
...
3. 신호·하위 구성과 산업 금액 행은 현재 집단에 조건을 추가해 이동합니다. 산업 금액 표의 **전체 시장**, 유형 기여 표의 **전체 유형**은 전체 Dashboard로 이동합니다.
...
8. **시장 규모 ₩**를 선택하면 지도 면적과 Discovery Radar를 금액으로 탐색합니다. 지출 범위를 선택하고 각 Dashboard에서 관련 인구·관련 성인당 배분액·산업별 금액을 확인합니다.
...
work/venv/bin/python pipeline/calibrate_behaviors.py
...
- 상업적 유형의 정의는 원 서술에서 추출한 행동·욕구·채널 조합입니다. 일부 소속은 NIA·소비자원 조사 비율을 이식한 모델로 보정합니다. 시장과 인구특성은 유형의 정의에서 제외했습니다.
- 소속은 서술 및 조사 비율로 보정한 모델의 0/1 조합이며 중복 가능합니다. 실제 사람의 행동 관측이 아닙니다. 현재 커버리지와 중복률은 엔진 결과를 화면에 표시합니다.
- 인구 범위는 원 서술 ±30%, 외부 보정 최소 ±40%, 원 서술 1,000건 미만은 ±50%의 민감도 범위입니다. 통계적 신뢰구간이 아닙니다.
- 원천은 합성 서술입니다. 실제 지출, 소득, 거래 빈도, 시장 성장, 경쟁, 지불의향을 측정하지 않습니다. 해당 미확보 항목을 0으로 만들어 점수에 넣지 않습니다.
- 금액 엔진은 기존 출처가 확인된 월 지출 구간을 연환산하고, 연령별 참여율 및 관측 소비 신호로 배분합니다. 금액 계수·범위·출처는 `config/market-value.json`과 `config/external-spend.json`에 있습니다. 새로운 지출 기준은 대상·단위·기간·중복 항목 검토 후 연결해야 합니다.

## 문서와 데이터

- [v0.4 외부 소비 기준·인구 보정·제한](docs/EXTERNAL_SPEND_AND_POPULATION.md)
- [현재 52개 유형·20개 시장 전수 감사](data/revision-sanity.json)
