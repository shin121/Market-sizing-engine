...
- `scripts/extract-consumer-research.py` verifies the hash and extracts only numeric
  ...
  survey records are not shipped. Fifteen tables / thirty PDF pages are retained.
...
| `online_channels` | PC/mobile/SNS/C2C use %, monthly mean frequency, problem % | Use: all respondents. Frequency/problems: users of that channel | Extracted and validated; not yet connected to new profiles |
| `offline_channels` | Department store/mart/convenience store/traditional market use, frequency, problems | Same channel-specific distinction | Extracted; not yet connected |
...
- Base ±30% per conditional factor is a sensitivity assumption, not a survey CI.
...
The 71-test Atlas suite covers denominator correction, source nulls and last-column
