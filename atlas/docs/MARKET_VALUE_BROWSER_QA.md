...
> 이 문서는 v0.4 브라우저 검증 기록입니다. 현재 산식·외부 인구 보정·13/20개 산업 범위는 [개정 문서](EXTERNAL_SPEND_AND_POPULATION.md), 현재 전수 검증은 [research-demand-sanity.json](../data/research-demand-sanity.json)을 사용합니다.
...
실제 유형 `arc_premium_recovery`(휴식·보상 프리미엄선택형)를 사용했다. 요구사항의 예시 이름이나 숫자를 별도 데이터로 만들지 않았다. 금액 기준이 있는 음악·콘텐츠로 이동했고, 다른 7개 산업의 결손/가구 단위 상태도 화면에서 확인했다.
