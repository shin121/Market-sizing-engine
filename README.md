...
The local Production runtime now reads migrations `001`–`024` from fixture-free `market_engine_phase3r_production`. Authenticated health proves 24 domains, 90 Primary Subtypes, 1,440 Archetypes, 1,536 Axis values, 480 Features, 240 Behaviors, Gold Query 10/10, primary `not_estimable` 0, Production fixture 0, and missing display name 0. Migration `024` adds 91,091 identifier-free weighted joint cells across person/household/establishment/enterprise, 253 queryable calibrated dimension conditions, and direct nested AND/OR/NOT evaluation without reading raw Persona rows at runtime. Gold Query 10/10 also expose their registered related-spend Low/Base/High with period and per-entity boundaries. A 116-table scan proves whole-Production fixture-like rows 0. The current gates are Python 70 passed/12 skipped, engine validation 46/46, web unit 280/280, PostgreSQL integration 51/51 in disposable and fresh-restore `_test` databases, typecheck, zero-warning lint, and a Next.js 16.3.2 Production build. The write-capable Browser suite passes 25 cases with 2 intentional viewport skips in an automatically removed clone while preserving the Production DoD fingerprint; the natural-language trace now confirms a structured Gold Query and opens its numeric snapshot. A separate repeatable read-only Production Playwright contract passes 4 cases with 2 intentional project-scoped skips across Explorer/Sizing/Research on desktop/tablet/mobile, a 24/24 Domain sweep, axe/overflow/fixture/internal-status checks, and a clean browser console. Browser-native Archetype row virtualization is computed-style verified across all three viewports. The authenticated loopback Production performance audit passes all 10 API/SSR p95 limits; Explorer SSR p95 is 15.397 ms and Archetype SSR p95 is 31.311 ms. The v2 deployment archive also restores 91,091 weighted cells over TCP/TLS as a non-superuser database owner, reapplies target-owner runtime ACLs, and passes the same 51 database tests through Node `verify-full`.

Use the current [architecture](docs/phase3r_7r_workbench_architecture.md), [API contract](docs/phase3r_7r_api.md), [data dictionary](docs/phase3r_7r_data_dictionary.md), [operations runbook](docs/phase3r_7r_operations_runbook.md), [Research workflow](docs/phase3r_7r_research_workflow.md), [user guide](docs/phase3r_7r_user_guide.md), [coverage report](reports/phase3r_7r_data_coverage.json), [deployment artifact manifest](reports/phase3r_7r_deployment_artifact.json), and [Product DoD audit](reports/phase3r_7r_product_dod_audit.json). A live OpenAI request still requires explicit approval to transmit its prompt/target/baseline payload; a cloud URL requires a managed PostgreSQL target because the verified database uses a local Unix socket.
...
The OpenAI credential is server-only and is never printed or committed. A real Research Job exists in the truthful `configuration_required` state; no fabricated provider output is stored. Key reuse has been selected, but transmitting the exact research prompt, target segment, and baseline to an external provider remains a separate explicit-approval gate. Local workers and the separately authenticated serverless run-once route share the same durable queue and retry contract.
